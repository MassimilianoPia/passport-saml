...
SAML.prototype.initialize = function(options) {
  ...
  if (
    ...
    ["exact", "minimum", "maximum", "better"].indexOf(options.RACComparison) ===
      -1
  ) {
    ...
    .then(function() {
      var request = {
        "samlp:AuthnRequest": {
          ...
          "saml:Issuer": {
            ...
            "@NameQualifier": self.options.nameQualifier, // required by spid
            "@Format": self.options.format // required by spid
          }
        }
      };
      ...
      if (self.options.identifierFormat) {
        request["samlp:AuthnRequest"]["samlp:NameIDPolicy"] = {
          ...
          //'@AllowCreate': 'true' //Not Allowed in Spid
        };
      }
...
SAML.prototype.generateLogoutRequest = function(req) {
  ...
  return Q.ninvoke(this.cacheProvider, "save", id, instant).then(function() {
...
SAML.prototype.getAuthorizeUrl = function(req, options, callback) {
  ...
  self.generateAuthorizeRequest(req, self.options.passive, function(
    ...
    self.requestToUrl(
      ...
      null,
      operation,
      self.getAdditionalParams(req, operation, overrideParams),
      callback
    );
  });
};
...
SAML.prototype.getLogoutUrl = function(req, options, callback) {
  var self = this;
  return self.generateLogoutRequest(req).then(function(request) {
    var operation = "logout";
    var overrideParams = options ? options.additionalParams || {} : {};
    return self.requestToUrl(
      request,
      null,
      operation,
      self.getAdditionalParams(req, operation, overrideParams),
...
SAML.prototype.validateInResponseTo = function(inResponseTo) {
  if (this.options.validateInResponseTo) {
    if (inResponseTo) {
      return Q.ninvoke(this.cacheProvider, "get", inResponseTo).then(function(
        result
      ) {
    ...
    parser.parseString(inflated, function(err, doc) {
      ...
      Q.fcall(function() {
        return samlMessageType === "SAMLResponse"
          ? self.verifyLogoutResponse(doc)
          : self.verifyLogoutRequest(doc);
      })
        .then(function() {
...
SAML.prototype.hasValidSignatureForRedirect = function(
  samlMessageType,
  container
) {
  ...
  if (signature && this.options.cert) {
    var urlString =
      samlMessageType + "=" + encodeURIComponent(container[samlMessageType]);

    if (container.RelayState) {
      urlString += "&RelayState=" + encodeURIComponent(container.RelayState);
    }
...
function validateSignatureForRedirect(urlString, signature, alg, cert) {
  var supportedAlgs = crypto.getHashes().filter(function(h) {
    return new RegExp(h).test(alg);
  });
  ...
  if (this.options.idpIssuer) {
    ...
    if (issuer) {
      if (issuer[0]._ !== this.options.idpIssuer)
        throw "Unknown SAML issuer. Expected: " +
...
SAML.prototype.processValidlySignedAssertion = function(
  ...
  Q.ninvoke(parser, "parseString", xml)
    ...
    .then(function() {
      ...
      profile.getAssertion = function() {
        return parsedAssertion;
      };
      profile.getSamlResponseXml = function() {
